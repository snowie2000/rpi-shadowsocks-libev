...
cares_ver=1.13.0
# set path
prefix_path='/opt/ss-arm'
# mips use mipel-linux-gnu, mips64 use mips64-linux-gnuabi64, default is mips
if [ $ARCHITECH == "arm64" ]
then
	export PATH=$PATH:/usr/local/rpi/tools-master/arm-bcm2708/gcc-linaro-arm-linux-gnueabihf-raspbian-x64/bin
    host=arm-linux-gnueabihf
    strip=arm-linux-gnueabihf-strip
else
	export PATH=$PATH:/usr/local/rpi/tools-master/arm-bcm2708/gcc-linaro-arm-linux-gnueabihf-raspbian/bin
    host=arm-linux-gnueabihf
    strip=arm-linux-gnueabihf-strip
fi
